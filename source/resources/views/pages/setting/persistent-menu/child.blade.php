...
@section('content')
    <div class="section no-pad-bot">
        <div class="container">
            <form class="row card-panel" method="POST" action="{{ route('setting.persistent-menu.store') }}">
                <a href="{{ route('setting.persistent-menu.index') }}" class="btn"><i class="material-icons">chevron_left</i></a>
                <input hidden name="level_menu" value="{{((int)$parent->level_menu) + 1}}">
                <input hidden name="persistent_id" value="{{ $parent->_id }}">
                @csrf
                <h3>Persistent menu child <span class="orange-text">{{ ((int)$parent->level_menu) + 1 }}</span> <span
                        class="orange-text">{{ $parent->title }}</span></h3>
                <?php $key = 0; ?>
                @foreach($persistent_menus as $key => $persistent_menu)
                    <div class="col s4" style="min-height: 450px">
                        <div>
                            <i class="material-icons">filter_{{ $key + 1 }}</i>
                            Item {{ $key + 1 }}
                            <a href="#call-modal" class="call-modal modal-trigger" data-id="{{ $persistent_menu->_id }}"
                        ...
                        <div class="element">
                            <div class="row">
                                <div class="input-field col s12">
                                    <input type="text" class="validate title" name="title[]" maxlength="20"
                                           autocomplete="off" data-length="20" value="{{ $persistent_menu->title }}">
                            ...
                            <div class="row url_ {{ $persistent_menu->type !== 'url' ? 'display-none' : '' }}">
                                <div class="input-field col s12">
                                    <input type="url" name="url[]" class="validate" value="{{ $persistent_menu->url }}">
                ...
                @endforeach
                @for($i = $persistent_menus->count(); $i < 5; $i++)
                    <div class="col s4" style="min-height: 450px">
                        <div>
