...
@section('content')
    <div class="section no-pad-bot">
        <div class="row">
            ...
            <div id="data-persistent-parent" class="w-33 position-relative">
                <div class="form-control">
                    <span class="text-secondary">Nhập tin nhắn...</span>
                </div>
                <div id="data-parent">
                    @foreach($menu_1 as $value)
                        <div class="form-control" data-id="{{$value->id}}">
                            <span class="text-dark font-weight-bold">{{$value['title']}}</span>
                            @switch($value['type'])
                                @case('nested')
                                <div class="position-relative">
                                    <div class="position-absolute" style="right: 0; bottom: 0;">
                                        <a href="#changeSubmenu"
                                           onclick="changeSubMenu(1, {{$value->id}})"
                                           data-show-sub-menu-parent="false">Thay đổi item sub
                                            menu<span
                                                class="mdi mdi-chevron-right mdi-24px position-relative"
                                                style="bottom: -5px"></span></a>
                                    </div>
                                    <a href="##$$$"
                                       class="mdi mdi-comment-remove mdi-18px text-danger position-absolute custom-remove-item"
                                       onclick="remove('{{$value->id}}', '{{$value->title}}')"></a>
                                </div>

                                <div class="w-100 position-absolute custom-element-child d-none"
                                     id="key-child-1-{{$value->id}}" data-show-child-1="false">
                                    <div class="form-control">
                                        <a href="####" onclick="backMenu(1, {{$value->id}})"><span
                                                class="mdi mdi-chevron-left"></span>Quay
                                            lại</a>
                                        <span class="text-center">{{$value['title']}}</span>
                                    </div>
                                    <div id="data-child-1-{{$value->id}}">
                                        @foreach($menu_2 as $item)
                                            @if($item->persistent_id == $value->id)
                                                <div class="form-control" data-id="{{$item->id}}">
                                                    ...
                                                    @switch($item['type'])
                                                        ...
                                                        <div class="position-relative">
                                                            <div class="position-absolute"
                                                                ...
                                                                <a href="##$$$"
                                                                   onclick="changeSubMenu(2, {{$item->id}})"
                                                                   data-show-sub-menu-parent="false">Thay
                                                                    đổi
                                                                    item sub menu<span
                                                                        class="mdi mdi-chevron-right mdi-24px position-relative"
                                                                        style="bottom: -5px"></span></a>
                                                            ...
                                                            <a href="##$$$"
                                                               ...
                                                               class="mdi mdi-comment-remove mdi-18px text-danger position-absolute custom-remove-item"></a>
                                                        </div>

                                                        <div
                                                            class="w-100 position-absolute custom-element-child d-none"
                                                            id="key-child-2-{{$item->id}}"
                                                            data-show-child-2="false">
                                                            <div class="form-control">
                                                                <a href="####"
                                                                   onclick="backMenu(2, {{$item->id}})"><span
                                                                        class="mdi mdi-chevron-left"></span>Quay
                                                                    lại</a>
                                                                <span
                                                                    class="text-center">{{$item['title']}}</span>
                                                            </div>
                                                            <div id="data-child-2-{{$item->id}}">
                                                                @foreach($menu_3 as $val)
                                                                    @if($val->persistent_id == $item->id)
                                                                        <div class="form-control"
                                                                             data-id="{{$val->id}}">
                                                                                        <span
                                                                                            class="text-dark font-weight-bold">{{$val['title']}}</span>
                                                                            @switch($val['type'])
                                                                                @case('nested')
                                                                                <div
                                                                                    ...
                                                                                    <a href="##$$$"
                                                                                       onclick="remove('{{$val->id}}', '{{$val->title}}')"
                                                                                       class="mdi mdi-comment-remove mdi-18px text-danger position-absolute"
                                                                                       style="right: -20px; bottom: -5px"></a>
                                                                                </div>
                                                                                @break
                                                                                @case('postback')
                                                                                <div
                                                            ...
                                                            <button type="button"
                                                                    class="btn btn-gradient-info btn-lg btn-block"
                                                                    onclick="btnAddItem('child-2', {{$item['id']}})">
                                                                <i class="mdi mdi-menu float-left"></i>Thêm
                                                                item
                                                            </button>
                                                            <div class="form-control d-none"
                                                                 id="add-item-child-2-{{$item->id}}"
                                                                 data-show-add-item="false"
                                                                 data-lvl-menu="3"></div>
                                                        </div>
                                                        @break
                                                        @case('postback')
                                    ...
                                    <button type="button"
                                            class="btn btn-gradient-info btn-lg btn-block"
                                            onclick="btnAddItem('child-1', {{$value['id']}})">
                                        <i class="mdi mdi-menu float-left"></i>Thêm item
                                    </button>
                                    <div class="form-control d-none"
                                         id="add-item-child-1-{{$value['id']}}"
                                         data-show-add-item="false" data-lvl-menu="2"
                                         data-id="{{$value->id}}"></div>
                                </div>
                                @break
                                @case('postback')
                                <div class="position-relative">
                                    <a href="##$$$"
                                       onclick="remove('{{$value->id}}', '{{$value->title}}')"
                                       class="mdi mdi-comment-remove mdi-18px text-danger position-absolute custom-remove-item"></a>
                                </div>
                                @break
                                @case('web_url')
                ...
                <button type="button" class="btn btn-gradient-info btn-lg btn-block"
                        onclick="btnAddItem('parent')" id="btn-persistent-parent">
                    <i class="mdi mdi-menu float-left"></i>Thêm item
                </button>
                <div class="form-control d-none" id="add-item" data-show-add-item="false"
                     data-lvl-menu="1"></div>
            </div>
        </div>
    </div>
